...
def process_special_word(text):
    ...
    text_lst = text.split()
    i= 0
    if 'không' in text_lst:   # => Cái này chỉ mới def cho chữ không. Làm tương tự cho các từ khác như chả, chẳng,...
        while i <= len(text_lst) - 1:
            word = text_lst[i]
            #print(word)
            #print(i)
...
# Chay dữ liệu NLP
def nlp_vietnamese(df, col_name, columns= ['content_2']):
    df_new = df[col_name].dropna()
    #print(df_new)
    lsts = []
    for i in df_new.values:
        # Bước 1: Tiền xử lý dữ liệu thô
        document = process_text(i,emoji_dict,teen_dict,wrong_lst)
